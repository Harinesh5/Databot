...
def generate_count_plot(df, x_col, hue_col):
    try:
        logger.info(f"Generating count plot for {x_col} with hue {hue_col}")
        ...
        st.pyplot(plt)
        logger.info("Count plot generated successfully")
        return plt.gcf()  # Return the figure for chat history
    except Exception as e:
        logger.error(f"Error generating count plot: {str(e)}")
...
def search_general_knowledge(query):
    try:
        ...
        logger.info(f"General response received")
